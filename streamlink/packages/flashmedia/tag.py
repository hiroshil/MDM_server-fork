...
class TypeFlags(Union):
    ...
    class Bits(BigEndianStructure):
        _fields_ = [('rsv1', c_uint8, 5), ('audio', c_uint8, 1), ('rsv2', c_uint8, 1), ('video', c_uint8, 1)]
...
class TagFlags(Union):
    ...
    class Bits(BigEndianStructure):
        _fields_ = [('rsv', c_uint8, 2), ('filter', c_uint8, 1), ('type', c_uint8, 5)]
...
class AudioFlags(Union):
    ...
    class Bits(BigEndianStructure):
        _fields_ = [('codec', c_uint8, 4), ('rate', c_uint8, 2), ('bits', c_uint8, 1), ('type', c_uint8, 1)]
...
class VideoFlags(Union):
    ...
    class Bits(BigEndianStructure):
        _fields_ = [('type', c_uint8, 4), ('codec', c_uint8, 4)]
...
class Header(Packet):
    ...
    def __repr__(self):
        ...
        return reprformat.format(version=self.version, offset=self.data_offset, has_audio=self.has_audio, has_video=self.has_video)
    ...
    def _deserialize(cls, io):
        ...
        return Header(version, bool(flags.bit.audio), bool(flags.bit.video), offset, tag0_size)
    ...
    def _deserialize_from(cls, buf, offset):
        ...
        if head != b'FLV':
            raise FLVError('Invalid FLV header')
        flags = TypeFlags()
        (version, flags.byte, tag0_offset, tag0_size) = unpack_many_from(buf, offset, (U8, U8, U32BE, U32BE))
        rval = Header(version, bool(flags.bit.audio), bool(flags.bit.video), tag0_offset, tag0_size)
        offset += 10
        return (rval, offset)
    ...
    def _serialize_into(self, packet, offset):
        ...
        offset = pack_many_into(packet, offset, (U8, U8, U32BE, U32BE), (self.version, self.flags.byte, self.data_offset, self.tag0_size))
        return offset

class Tag(Packet):
    exception = FLVError

    def __init__(self, typ=TAG_TYPE_SCRIPT, timestamp=0, data=None, streamid=0, filter=False, padding=None):
        self.flags = TagFlags()
    ...
    def __repr__(self):
        ...
        return reprformat.format(type=self.type, timestamp=self.timestamp, streamid=self.streamid, filter=self.filter, data=repr(self.data))
    ...
    def _deserialize(cls, io, strict=False, raw_data=False):
        ...
        (flagb, data_size, timestamp, timestamp_ext, streamid) = unpack_many_from(header, 0, (U8, U24BE, U24BE, U8, U24BE))
        flags = TagFlags()
        flags.byte = flagb
        ...
        if data_size > 0 and not raw_data:
            ...
            padding = b''
        tag = Tag(flags.bit.type, timestamp, data, streamid, bool(flags.bit.filter), padding)
    ...
    @classmethod
    def _deserialize_from(cls, buf, offset, strict=False, raw_data=False):
        (flagb, data_size, timestamp, timestamp_ext, streamid) = unpack_many_from(buf, offset, (U8, U24BE, U24BE, U8, U24BE))
        offset += 11
        flags = TagFlags()
        flags.byte = flagb
        timestamp |= timestamp_ext << 24
    ...
    def _serialize(self, packet, strict=True):
        ...
        packet += U24BE(self.timestamp & 16777215)
        packet += U8(self.timestamp >> 24 & 127)
    ...
    def _serialize_into(self, packet, offset):
        offset = pack_many_into(packet, offset, (U8, U24BE, U24BE, U8, U24BE), (self.flags.byte, self.data_size, self.timestamp & 16777215, self.timestamp >> 24 & 127, self.streamid))
...
class AudioData(TagData):
    ...
    def __repr__(self):
        ...
        return reprformat.format(type=self.type, codec=self.codec, rate=self.rate, bits=self.bits, data=data)
    ...
    def _deserialize(cls, io):
        ...
        if flags.bit.codec == AUDIO_CODEC_ID_AAC:
            ...
            data = io.read()
        return cls(flags.bit.codec, flags.bit.rate, flags.bit.bits, flags.bit.type, data)
    ...
    def _deserialize_from(cls, buf, offset, buf_size=None):
        ...
        if flags.bit.codec == 10:
            ...
            data = buf[offset:offset + buf_size]
            offset += buf_size
        obj = cls(flags.bit.codec, flags.bit.rate, flags.bit.bits, flags.bit.type, data)
        return (obj, offset)
...
class VideoData(TagData):
    ...
    def _deserialize(cls, io):
        ...
        elif flags.bit.codec == VIDEO_CODEC_ID_AVC:
    ...
    def _deserialize_from(cls, buf, offset, buf_size=None):
        ...
        buf_size -= U8.size
        if flags.bit.type == VIDEO_FRAME_TYPE_COMMAND_FRAME:
            (data, offset) = VideoCommandFrame.deserialize_from(buf, offset, buf_size=buf_size)
        elif flags.bit.codec == VIDEO_CODEC_ID_AVC:
            (data, offset) = AVCVideoData.deserialize_from(buf, offset, buf_size=buf_size)
        else:
            data = buf[offset:offset + buf_size]
            offset += buf_size
...
class AVCVideoData(TagData):
    ...
    def __repr__(self):
        ...
        return reprformat.format(type=self.type, composition_time=self.composition_time, data=data)
    ...
    def _serialize_into(self, packet, offset):
        offset = pack_many_into(packet, offset, (U8, S24BE), (self.type, self.composition_time))
        offset = pack_bytes_into(packet, offset, self.data)
        return offset
...
TagDataTypes = {TAG_TYPE_AUDIO: AudioData, TAG_TYPE_VIDEO: VideoData, TAG_TYPE_SCRIPT: ScriptData}
__all__ = ['Header', 'Tag', 'FrameData', 'AudioData', 'AACAudioData', 'VideoData', 'VideoCommandFrame', 'AVCVideoData', 'ScriptData', 'TAG_TYPE_VIDEO', 'TAG_TYPE_AUDIO', 'TAG_TYPE_SCRIPT']
