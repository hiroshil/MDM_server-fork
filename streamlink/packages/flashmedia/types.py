...
(SCRIPT_DATA_TYPE_NUMBER, SCRIPT_DATA_TYPE_BOOLEAN, SCRIPT_DATA_TYPE_STRING, SCRIPT_DATA_TYPE_OBJECT, SCRIPT_DATA_TYPE_RESERVED, SCRIPT_DATA_TYPE_NULL, SCRIPT_DATA_TYPE_UNDEFINED, SCRIPT_DATA_TYPE_REFERENCE, SCRIPT_DATA_TYPE_ECMAARRAY, SCRIPT_DATA_TYPE_OBJECTEND, SCRIPT_DATA_TYPE_STRICTARRAY, SCRIPT_DATA_TYPE_DATE, SCRIPT_DATA_TYPE_LONGSTRING) = range(13)
SCRIPT_DATA_TYPE_AMF3 = 17
(AMF3_TYPE_UNDEFINED, AMF3_TYPE_NULL, AMF3_TYPE_FALSE, AMF3_TYPE_TRUE, AMF3_TYPE_INTEGER, AMF3_TYPE_DOUBLE, AMF3_TYPE_STRING, AMF3_TYPE_XML_DOC, AMF3_TYPE_DATE, AMF3_TYPE_ARRAY, AMF3_TYPE_OBJECT, AMF3_TYPE_XML, AMF3_TYPE_BYTE_ARRAY, AMF3_TYPE_VECTOR_INT, AMF3_TYPE_VECTOR_UINT, AMF3_TYPE_VECTOR_DOUBLE, AMF3_TYPE_VECTOR_OBJECT, AMF3_TYPE_DICT) = range(18)
AMF3_EMPTY_STRING = 1
AMF3_DYNAMIC_OBJECT = 11
...
class String(DynamicType):
    ...
    @classmethod
    def pack_into(cls, buf, offset, val, encoding='utf8', errors='ignore'):
        return pack_bytes_into(buf, offset, val.encode(encoding, errors))
...
class ScriptDataString(String):
    ...
    def pack_into(cls, buf, offset, val, *args, **kwargs):
        noffset = String.pack_into(buf, offset + cls.__size_primitive__.size, val, *args, **kwargs)
        cls.__size_primitive__.pack_into(buf, offset, noffset - offset - cls.__size_primitive__.size)
        return noffset
...
class ScriptDataECMAArray(ScriptDataObject):
    ...
    def pack_into(cls, buf, offset, val):
        ...
        return ScriptDataObject.pack_into(buf, offset + U32BE.size, val)
...
class ScriptDataValue(DynamicType, ScriptDataType):
    PrimitiveReaders = {SCRIPT_DATA_TYPE_NUMBER: ScriptDataNumber, SCRIPT_DATA_TYPE_BOOLEAN: ScriptDataBoolean, SCRIPT_DATA_TYPE_REFERENCE: ScriptDataReferenceStruct, SCRIPT_DATA_TYPE_DATE: ScriptDataDateStruct}
    DynamicReaders = {SCRIPT_DATA_TYPE_STRING: ScriptDataString, SCRIPT_DATA_TYPE_LONGSTRING: ScriptDataLongString, SCRIPT_DATA_TYPE_OBJECT: ScriptDataObject, SCRIPT_DATA_TYPE_ECMAARRAY: ScriptDataECMAArray, SCRIPT_DATA_TYPE_STRICTARRAY: ScriptDataStrictArray}
    ...
    def read(cls, fd, marker=None):
        ...
        if type_ == SCRIPT_DATA_TYPE_OBJECTEND:
            raise ScriptDataObjectEnd
        else:
            if type_ == SCRIPT_DATA_TYPE_NULL or type_ == SCRIPT_DATA_TYPE_UNDEFINED:
                return
    ...
    def unpack_from(cls, buf, offset):
        ...
        if type_ in ScriptDataValue.PrimitiveReaders:
...
class AMF3Integer(DynamicType, AMF3Type):
    ...
    def pack_into(cls, buf, offset, val):
        ...
        if val < 128:
            ...
            buf[offset] = val >> 7 & 127 | 128
            buf[offset + 1] = val & 127
            offset += 2
        elif val < 2097152:
            buf[offset] = val >> 14 & 127 | 128
            buf[offset + 1] = val >> 7 & 127 | 128
            buf[offset + 2] = val & 127
            offset += 3
        elif val < 1073741824:
            buf[offset] = val >> 22 & 127 | 128
            buf[offset + 1] = val >> 15 & 127 | 128
            buf[offset + 2] = val >> 8 & 127 | 128
    ...
    def read(cls, fd):
        (rval, byte_count) = (0, 0)
        byte = U8.read(fd)
        while byte & 128 != 0 and byte_count < 3:
            rval <<= 7
            rval |= byte & 127
            byte = U8.read(fd)
            byte_count += 1
...
class AMF3ObjectPacker(DynamicType, AMF3Type):
    ...
    def pack(cls, val, str_cache, object_cache, traits_cache):
        ...
        if val in object_cache:
            ...
            if traits in traits_cache:
                index = traits_cache.index(traits)
                chunks.append(AMF3Integer(index << 2 | 1))
            else:
                header = 3
                if traits.__dynamic__:
                    header |= 8
            ...
            for member in traits.__members__:
                value = getattr(val, member)
                value = AMF3Value.pack(value, str_cache=str_cache, object_cache=object_cache, traits_cache=traits_cache)
                chunks.append(value)
            if traits.__dynamic__:
                if isinstance(val, AMF3Object):
                    iterator = val.items()
        ...
        if header & 1 == 0:
            ...
            if header & 1 == 0:
                ...
                if len(class_name) == 0:
                    ...
                    traits = AMF3ObjectBase.create(class_name, externalizable, dynamic, members)
                    traits_cache.append(traits)
            values = OrderedDict()
            for member in traits.__members__:
                value = AMF3Value.read(fd, str_cache=str_cache, object_cache=object_cache, traits_cache=traits_cache)
                values[member] = value
            if traits.__dynamic__:
...
class AMF3DatePacker(DynamicType, AMF3Type):
    ...
    def pack(cls, val, cache):
        if val in cache:
            ...
            cache.append(val)
            chunks = [U8(AMF3_TYPE_NULL), AMF3Double(val.time)]
            return b''.join(chunks)
...
class AMF3Value(DynamicType):
    PrimitiveReaders = {AMF3_TYPE_DOUBLE: AMF3Double}
    DynamicReaders = {AMF3_TYPE_INTEGER: AMF3Integer}
    Readers = PrimitiveReaders.copy()
    Readers.update(DynamicReaders)
    ...
    def pack(cls, val, str_cache=None, object_cache=None, traits_cache=None):
        ...
        if isinstance(val, bool):
            ...
            chunks.append(AMF3ArrayPacker.pack(val, str_cache=str_cache, object_cache=object_cache, traits_cache=traits_cache))
        elif isinstance(val, string_types):
            chunks.append(U8(AMF3_TYPE_STRING))
            chunks.append(AMF3String.pack(val, cache=str_cache))
        elif isinstance(val, AMF3ObjectBase):
            chunks.append(U8(AMF3_TYPE_OBJECT))
            chunks.append(AMF3ObjectPacker.pack(val, str_cache=str_cache, object_cache=object_cache, traits_cache=traits_cache))
        elif isinstance(val, AMF3Date):
