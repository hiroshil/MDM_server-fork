...
class Box(Packet):
    ...
    def size(self):
        ...
        if size > 4294967295 or self.extended_size:
            size += 8
        return size
...
class BoxPayloadFTYP(BoxPayload):
    ...
    def __init__(self, major_brand='f4v', minor_version=0, compatible_brands=['isom', 'mp42', 'm4v']):
    ...
    @property
    def size(self):
        return 8 + len(self.compatible_brands)*4
...
class BoxPayloadMVHD(BoxPayload):
    ...
    def __init__(self, version=0, creation_time=0, modification_time=0, time_scale=1000, duration=0, rate=1.0, volume=1.0, matrix=[65536, 0, 0, 0, 65536, 0, 0, 0, 1073741824], next_track_id=0):
    ...
    def _deserialize(cls, io):
        ...
        return cls(version, creation_time, modification_time, time_scale, duration, rate, volume, matrix, next_track_id)
...
class SampleFlags(BoxPayload):
    ...
    class Flags(Union):
        ...
        class Bits(BigEndianStructure):
            _fields_ = [('reserved', c_uint8, 6), ('sample_depends_on', c_uint8, 2), ('sample_is_depended_on', c_uint8, 2), ('sample_has_redundancy', c_uint8, 2), ('sample_padding_value', c_uint8, 3), ('sample_is_difference_sample', c_uint8, 1), ('sample_degradation_priority', c_uint16, 16)]
    ...
    def _deserialize(cls, io):
        ...
        return cls(flags.bit.sample_depends_on, flags.bit.sample_is_depended_on, flags.bit.sample_has_redundancy, flags.bit.sample_padding_value, flags.bit.sample_is_difference_sample, flags.bit.sample_degradation_priority)

class BoxPayloadTREX(BoxPayload):

    def __init__(self, version, track_id, default_sample_description_index, default_sample_duration, default_sample_size, default_sample_flags):
        self.version = version
    ...
    @property
    def size(self):
        return 20 + self.default_sample_flags.size
    ...
    def _deserialize(cls, io):
        ...
        return cls(version, track_id, default_sample_description_index, default_sample_duration, default_sample_size, default_sample_flags)

class BoxPayloadTKHD(BoxPayload):

    def __init__(self, version=0, flags=1, creation_time=0, modification_time=0, track_id=1, duration=0, layer=0, alternate_group=0, volume=0.0, transform_matrix=[65536, 0, 0, 0, 65536, 0, 0, 0, 1073741824], width=0.0, height=0.0):
        self.version = version
        self.flags = flags
        self.creation_time = creation_time
        self.modification_time = modification_time
    ...
    def _deserialize(cls, io):
        ...
        return cls(version, flags, creation_time, modification_time, track_id, duration, layer, alternate_group, volume, transform_matrix, width, height)

class BoxPayloadMDHD(BoxPayload):

    def __init__(self, version=0, creation_time=0, modification_time=0, time_scale=1000, duration=0, language='eng'):
        self.version = version
        self.creation_time = creation_time
        self.modification_time = modification_time
        self.time_scale = time_scale
    ...
    def _deserialize(cls, io):
        ...
        U16BE.read(io)
        return cls(version, creation_time, modification_time, time_scale, duration, language)

class BoxPayloadHDLR(BoxPayload):

    def __init__(self, version=0, predefined=0, handler_type='vide', name=''):
        self.version = version
    ...
    def _deserialize(cls, io):
        ...
        return cls(version, predefined, handler_type, name)
...
class BoxPayloadVisualSample(BoxContainer):
    ...
    def __init__(self, data_reference_index=0, width=0, height=0, horiz_resolution=0.0, vert_resolution=0.0, frame_count=0, compressor_name='', depth=0, boxes=[]):
...
class BoxPayloadABST(BoxPayload):
    ...
    class Flags(Union):
        ...
        class Bits(BigEndianStructure):
            _fields_ = [('profile', c_uint8, 2), ('live', c_uint8, 1), ('update', c_uint8, 1), ('reserved', c_uint8, 4)]

        _fields_ = [('bit', Bits), ('byte', c_uint8)]

    def __init__(self, version, bootstrap_info_version, profile, live, update, time_scale, current_media_time, smpte_time_code_offset, movie_identifier, server_entry_table, quality_entry_table, drm_data, metadata, segment_run_table_entries, fragment_run_table_entries):
        self.version = version
    ...
    def _deserialize(cls, io):
        ...
        return cls(version, bootstrap_info_version, flags.bit.profile, flags.bit.live, flags.bit.update, time_scale, current_media_time, smpte_time_code_offset, movie_identifier, server_entry_table, quality_entry_table, drm_data, metadata, segment_run_table_entries, fragment_run_table_entries)
...
class BoxPayloadASRT(BoxPayload):
    ...
    def __init__(self, version, flags, quality_segment_url_modifiers, segment_run_entry_table):
        self.version = version
        self.flags = flags
    ...
    def _deserialize(cls, io):
        ...
        return cls(version, flags, quality_segment_url_modifiers, segment_run_entry_table)

class FragmentRunEntry(BoxPayload):

    def __init__(self, first_fragment, first_fragment_timestamp, fragment_duration, discontinuity_indicator):
    ...
    def _deserialize(cls, io):
        ...
        return cls(first_fragment, first_fragment_timestamp, fragment_duration, discontinuity_indicator)

class BoxPayloadAFRT(BoxPayload):

    def __init__(self, version, flags, time_scale, quality_segment_url_modifiers, fragment_run_entry_table):
        self.version = version
        self.flags = flags
        self.time_scale = time_scale
        self.quality_segment_url_modifiers = quality_segment_url_modifiers
    ...
    def _deserialize(cls, io):
        ...
        return cls(version, flags, time_scale, quality_segment_url_modifiers, fragment_run_entry_table)
...
PayloadTypes = {'ftyp': BoxPayloadFTYP, 'mvhd': BoxPayloadMVHD, 'trex': BoxPayloadTREX, 'tkhd': BoxPayloadTKHD, 'mdhd': BoxPayloadMDHD, 'hdlr': BoxPayloadHDLR, 'vmhd': BoxPayloadVMHD, 'dref': BoxPayloadDREF, 'url': BoxPayloadURL, 'stsd': BoxPayloadSTSD, 'mdat': BoxPayloadMDAT, 'abst': BoxPayloadABST, 'asrt': BoxPayloadASRT, 'afrt': BoxPayloadAFRT, 'skip': BoxPayloadSKIP, 'free': BoxPayloadFREE, 'moov': BoxPayloadMOOV, 'moof': BoxPayloadMOOF, 'mvex': BoxPayloadMVEX, 'mdia': BoxPayloadMDIA, 'minf': BoxPayloadMINF, 'meta': BoxPayloadMETA, 'mfra': BoxPayloadMFRA, 'stbl': BoxPayloadSTBL, 'trak': BoxPayloadTRAK, 'dinf': BoxPayloadDINF}
