...
from ..packages.flashmedia.tag import AudioData, AACAudioData, VideoData, AVCVideoData, VideoCommandFrame, Header, ScriptData, Tag
from ..packages.flashmedia.tag import AAC_PACKET_TYPE_SEQUENCE_HEADER, AVC_PACKET_TYPE_SEQUENCE_HEADER, AUDIO_CODEC_ID_AAC, VIDEO_CODEC_ID_AVC, TAG_TYPE_AUDIO, TAG_TYPE_VIDEO
    ...
    for (tag_index, tag) in enumerate(iter_flv_tags(fd)):
        if isinstance(tag.data, ScriptData) and tag.data.name == 'onMetaData':
            ...
            if tag.data.data.type == AVC_PACKET_TYPE_SEQUENCE_HEADER:
                avc_header = tag
        elif isinstance(tag.data, AudioData) and isinstance(tag.data.data, AACAudioData) and tag.data.data.type == AAC_PACKET_TYPE_SEQUENCE_HEADER:
            aac_header = tag
...
class FLVTagConcat(object):
    ...
    def __init__(self, duration=None, tags=[], has_video=True, has_audio=True, flatten_timestamps=False, sync_headers=False):
    ...
    def verify_tag(self, tag):
        ...
        if self.sync_headers and self.timestamps_sub:
        ...
        if isinstance(tag.data, AudioData):
            if isinstance(tag.data.data, AACAudioData):
                if tag.data.data.type == AAC_PACKET_TYPE_SEQUENCE_HEADER:
                    ...
                    if not self.audio_header_written:
                        return
            elif self.sync_headers and not self.headers_written:
                return
            ...
            if isinstance(tag.data.data, AVCVideoData):
                ...
                if isinstance(tag.data.data, VideoCommandFrame):
                    return
                if self.sync_headers and not self.headers_written:
                    return
        elif tag.data.name == 'onMetaData':
            if self.duration:
    ...
    def iter_chunks(self, fd=None, buf=None, skip_header=None):
        ...
        for tag in chain(analyzed_tags, tag_iterator):
            ...
            if self.flv_header_written or self.verify_tag(tag):
...
class FLVTagConcatWorker(Thread):
    ...
    def __init__(self, iterator, stream):
        ...
        self.concater = FLVTagConcat(stream.duration, stream.tags, **stream.concater_params)
    ...
    def run(self):
        for fd in self.stream_iterator:
            try:
                chunks = self.concater.iter_chunks(fd, skip_header=self.stream.skip_header)
                for chunk in chunks:
...
class FLVTagConcatIO(IOBase):
    ...
    def __init__(self, session, duration=None, tags=[], skip_header=None, timeout=30, **concater_params):
    ...
    def read(self, size=-1):
        ...
        return self.buffer.read(size, block=self.worker.is_alive(), timeout=self.timeout)
