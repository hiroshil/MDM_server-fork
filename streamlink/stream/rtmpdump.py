...
class RTMPStream(StreamProcess):
    __doc__ = """RTMP stream using rtmpdump.
    ...
    *Attributes:*
    ...
    def __init__(self, session, params, redirect=False, **kwargs):
        ...
        if self.session.options.get('subprocess-errorlog-path') or self.session.options.get('subprocess-errorlog'):
    ...
    def __repr__(self):
        return '<RTMPStream({0!r}, redirect={1!r}>'.format(self.parameters, self.redirect)

    def __json__(self):
        return dict(type=RTMPStream.shortname(), args=self.arguments, params=self.parameters)
