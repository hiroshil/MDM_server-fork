...
class Auth3TokenGenerator(TokenGenerator):
    ...
    def generate(self):
        ...
        res = self.stream.session.http.get(self.stream.swf, exception=StreamError)
...
class AkamaiHDStreamIO(io.IOBase):
    ...
    StatusComplete = 3
    StatusError = 4
    Errors = {1: 'Stream not found', 2: 'Track not found', 3: 'Seek out of bounds', 4: 'Authentication failed', 5: 'DVR disabled', 6: 'Invalid bitrate test'}
    ...
    def send_control(self, cmd, headers=None, **params):
        if not headers:
            headers = {}
        url = self.ControlURLFormat.format(host=self.host, streamname=self.streamname)
        headers['x-Akamai-Streaming-SessionID'] = self.sessionid
        params = self._create_params(cmd=cmd, **params)
        return self.session.http.post(url, headers=headers, params=params, data=self.ControlData, exception=StreamError)
    ...
    def _create_params(self, **extra):
        params = dict(v=self.Version, fp=self.FlashVersion, r=cache_bust_string(5), g=self.guid)
        params.update(extra)
        return params
...
class AkamaiHDStream(Stream):
    ...
    *Attributes:*
    ...
    def __repr__(self):
        return '<AkamaiHDStream({0!r}, swf={1!r})>'.format(self.url, self.swf)

    def __json__(self):
        return dict(type=AkamaiHDStream.shortname(), url=self.url, swf=self.swf)

    def open(self):
        stream = AkamaiHDStreamIO(self.session, self.url, self.swf, self.seek)
