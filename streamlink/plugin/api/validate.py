...
__all__ = ['any', 'all', 'filter', 'get', 'getattr', 'hasattr', 'length', 'optional', 'transform', 'text', 'union', 'union_get', 'url', 'startswith', 'endswith', 'contains', 'xml_element', 'xml_find', 'xml_findall', 'xml_findtext', 'validate', 'Schema', 'SchemaContainer']
text = str
_all = all
_getattr = getattr
_hasattr = hasattr
...
class any(tuple):
    __doc__ = 'At least one of the schemas must be valid.'

    def __new__(cls, *args):
        return super().__new__(cls, args)

class all(tuple):
    __doc__ = 'All schemas must be valid.'
...
class transform:
    __doc__ = 'Applies function to value to transform it.'

    def __init__(self, func):
        self.func = func

class optional:
    __doc__ = 'An optional key used in a dict or union-dict.'

    def __init__(self, key):
        self.key = key

class union(SchemaContainer):
    __doc__ = 'Extracts multiple validations based on the same value.'

class attr(SchemaContainer):
    __doc__ = "Validates an object's attributes."
...
def length(length):
    ...
    def min_len(value):
        if not len(value) >= length:
            raise ValueError('Minimum length is {0} but value is {1}'.format(length, len(value)))
        return True
...
def validate_type(schema, value):
    if isinstance(value, schema):
        return value
    raise ValueError("Type of {0!r} should be '{1}' but is '{2}'".format(value, schema.__name__, type(value).__name__))
...
def validate_attr(schema, value):
    ...
    for (attr, schema) in schema.schema.items():
        if not _hasattr(value, attr):
            raise ValueError("Attribute '{0}' not found on object '{1}'".format(attr, value))
...
class Schema:
    __doc__ = 'Wraps a validator schema into a object.'
