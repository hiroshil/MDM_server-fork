...
COOKIES = {'family_filter': 'off', 'ff': 'off'}
...
""", re.VERBOSE)
_media_schema = validate.Schema(validate.any({'error': {'title': validate.text}}, {'qualities': validate.any({validate.text: validate.all([{'type': validate.text, 'url': validate.url()}])})}))
_live_id_schema = validate.Schema({'total': int, 'list': validate.any([], [{'id': validate.text}])})
...
class DailyMotion(Plugin):
    ...
    def get_live_id(self, username):
        ...
        '''
        params = {'flags': 'live_onair'}
        api_user_videos = USER_INFO_URL.format(username) + '/videos'
        try:
            res = self.session.http.get(api_user_videos.format(username), params=params)
