...
class YouTube(Plugin):
    _re_ytInitialData = re.compile('var\\s+ytInitialData\\s*=\\s*({.*?})\\s*;\\s*</script>', re.DOTALL)
    _re_ytInitialPlayerResponse = re.compile('var\\s+ytInitialPlayerResponse\\s*=\\s*({.*?});\\s*var\\s+meta\\s*=', re.DOTALL)
    _re_mime_type = re.compile('^(?P<type>\\w+)/(?P<container>\\w+); codecs="(?P<codecs>.+)"$')
    _url_canonical = 'https://www.youtube.com/watch?v={video_id}'
    _url_channelid_live = 'https://www.youtube.com/channel/{channel_id}/live'
    adp_video = {137: '1080p', 299: '1080p60', 264: '1440p', 308: '1440p60', 266: '2160p', 315: '2160p60', 138: '2160p', 302: '720p60', 135: '480p', 133: '240p', 160: '144p'}
    adp_audio = {140: 128, 141: 256, 171: 128, 249: 48, 250: 64, 251: 160, 256: 256, 258: 258}
    match = None
    ...
    def can_handle_url(cls, url):
        cls.match = _url_re.match(url)
        return cls.match is not None
    ...
    def _schema_playabilitystatus(cls, data):
        schema = validate.Schema({'playabilityStatus': {'status': str, validate.optional('reason'): str}}, validate.get('playabilityStatus'), validate.union_get('status', 'reason'))
        return validate.validate(schema, data)

    @classmethod
    def _schema_videodetails(cls, data):
        schema = validate.Schema({'videoDetails': {'videoId': str, 'author': str, 'title': str, validate.optional('isLive'): validate.transform(bool), validate.optional('isLiveContent'): validate.transform(bool), validate.optional('isLiveDvrEnabled'): validate.transform(bool), validate.optional('isLowLatencyLiveStream'): validate.transform(bool), validate.optional('isPrivate'): validate.transform(bool)}}, validate.get('videoDetails'))
        videoDetails = validate.validate(schema, data)
        return validate.validate(validate.union_get('videoId', 'author', 'title', 'isLive'), videoDetails)

    @classmethod
    def _schema_streamingdata(cls, data):
        schema = validate.Schema({'streamingData': {validate.optional('hlsManifestUrl'): str, validate.optional('formats'): [validate.all({'itag': int, 'qualityLabel': str, validate.optional('url'): validate.url(scheme='http')}, validate.union_get('url', 'qualityLabel'))], validate.optional('adaptiveFormats'): [validate.all({'itag': int, 'mimeType': validate.all(str, validate.transform(cls._re_mime_type.search), validate.union_get('type', 'codecs')), validate.optional('url'): validate.url(scheme='http'), validate.optional('qualityLabel'): str}, validate.union_get('url', 'qualityLabel', 'itag', 'mimeType'))]}}, validate.get('streamingData'), validate.union_get('hlsManifestUrl', 'formats', 'adaptiveFormats'))
    ...
    def _create_adaptive_streams(self, adaptive_formats):
        ...
        for (url, label, itag, mimeType) in adaptive_formats:
            ...
            if stream_type == 'audio' and (best_audio_itag is None or self.adp_audio[itag] > self.adp_audio[best_audio_itag]):
    ...
    def _get_data_from_api(self, res):
        ...
        if _i_video_id is None:
            for link in itertags(res.text, 'link'):
                ...
                return
        try:
            _i_api_key = re.search('"INNERTUBE_API_KEY":\\s*"([^"]+)"', res.text).group(1)
        except AttributeError:
            _i_api_key = 'AIzaSyAO_FJ2SlqU8Q4STEHLGCilw_Y9_11qcW8'
        try:
            _i_version = re.search('"INNERTUBE_CLIENT_VERSION":\\s*"([\\d\\.]+)"', res.text).group(1)
        except AttributeError:
            _i_version = '1.20210616.1.0'
        res = self.session.http.post('https://www.youtube.com/youtubei/v1/player', headers={'Content-Type': 'application/json'}, params={'key': _i_api_key}, data=json.dumps({'videoId': _i_video_id, 'context': {'client': {'clientName': 'WEB_EMBEDDED_PLAYER', 'clientVersion': _i_version, 'platform': 'DESKTOP', 'clientFormFactor': 'UNKNOWN_FORM_FACTOR', 'browserName': 'Chrome'}, 'user': {'lockedSafetyMode': 'false'}, 'request': {'useSsl': 'true'}}}))
    ...
    def _get_streams(self):
        ...
        if self.match.group('channel'):
            if not self.match.group('channel_live'):
