...
class UnpackingError(Exception):
    __doc__ = """Badly packed source or general error. Argument is a
...
def extract(string):
    r = re.search('\\s+?(eval\\(function\\(p,a,c,k,e,d\\).+)\\s+?', string)
    if r:
        return r.group(1)
    return ''
...
def unpack(source):
    ...
    try:
        ...
        source = re.sub('\\b\\w+\\b', lookup, payload, flags=re.ASCII)
...
class Unbaser(object):
    __doc__ = """Functor for a given base. Will efficiently convert
